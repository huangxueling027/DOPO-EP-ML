...
Final role / 最终定位：
    BDE_opt.py 是 BDE.py 的“优化测试工具”，不是论文正式主模型。
    它用于检查是否存在比 BDE.py 更好的模型组合；如果没有稳定超过 BDE.py，最终仍以 BDE.py 为准。

当前结论 / Current decision：
    已完成快速测试和 25 runs 验证：
    - 5-fold × 1 repeat 时，ExtraTrees_sqrt_local/k=1200 曾达到 R2≈0.7834；
    - 但 5-fold × 5 repeats 后，最佳约为 R2≈0.7683，未稳定超过 BDE.py 的 R2≈0.7720。
    因此本文件默认改为“快速验证模式”，避免每次误跑 wide 大搜索。
...
Default training / 默认训练：
    ...
    XGB          = off by default
    Stacking     = off by default
    wide pool    = off by default

Why this change / 为什么这样修改：
    - 默认 wide 模式过慢，且 25 runs 验证没有稳定提升；
    - 默认 raw 优于继续默认 raw + bond_residual；
    - 默认 K=1200/1400 已覆盖当前最有希望区域；
    - 默认关闭 XGB/Stacking 可以避免运行时间过长；
    - 如需重新大搜索，可以通过环境变量手动开启。

Recommended usage / 推荐用法：
    1) 日常正式结果：运行 BDE.py，不运行 BDE_opt.py。
    2) 小范围验证：直接运行 BDE_opt.py。
    3) 重新大搜索：手动设置 wide 环境变量。

Examples on Windows Git Bash / VS Code bash：
    # 默认快速验证
    python 07_BDE/BDE_opt.py

    # 更快试跑，只做 1 次重复
    export DOPO_BDE_N_REPEATS=1
    python 07_BDE/BDE_opt.py

    # 如确实需要 wide 搜索，再手动开启
    export DOPO_BDE_R2_SEARCH_LEVEL=wide
    export DOPO_BDE_WIDE_MODEL_POOL=1
    export DOPO_BDE_USE_XGB=1
    export DOPO_BDE_USE_STACKING=1
    export DOPO_BDE_RUN_STANDALONE_XGB=1
    export DOPO_BDE_TARGET_MODES=raw,bond_residual
    export DOPO_BDE_K_LIST_PC_PN=800,1000,1100,1200,1300,1400,1600,1800,2000,2400
    python 07_BDE/BDE_opt.py

Prediction / 预测：
    预测新分子优先使用 BDE.py 训练出的正式模型。
    BDE_opt.py 保存的模型只用于调参验证，不建议作为论文主结果，除非其 25 runs summary 稳定超过 BDE.py。
"""
...
OUTDIR = Path(os.environ.get("DOPO_BDE_RESULTS", str(THIS_DIR / "results" / "BDE_opt")))
...
USE_XGB = HAS_XGB and _env_flag("DOPO_BDE_USE_XGB", "0")
...
FAST_N_TREES = int(os.environ.get("DOPO_BDE_FAST_N_TREES", "320"))
SLOW_N_TREES = int(os.environ.get("DOPO_BDE_SLOW_N_TREES", "900"))
FINAL_MODEL_SEED = int(os.environ.get("DOPO_BDE_FINAL_SEED", "42"))

# v10.5 R2 optimizer switches.
# wide mode is slower than v10.3 but explores more configurations that may improve mean R2.
R2_SEARCH_LEVEL = os.environ.get("DOPO_BDE_R2_SEARCH_LEVEL", "fast").strip().lower()
WIDE_MODEL_POOL = _env_flag("DOPO_BDE_WIDE_MODEL_POOL", "0")
USE_STACKING = _env_flag("DOPO_BDE_USE_STACKING", "0")
USE_WEIGHTED_VOTE = _env_flag("DOPO_BDE_USE_WEIGHTED_VOTE", "1")
RUN_LINEAR_MODELS = _env_flag("DOPO_BDE_RUN_LINEAR_MODELS", "0")
RUN_SLOW_KERNEL = _env_flag("DOPO_BDE_RUN_SLOW_KERNEL", "0")
RUN_STANDALONE_XGB = _env_flag("DOPO_BDE_RUN_STANDALONE_XGB", "0")
...
R2_BOOST_NOTE = (
    "BDE_opt clean: default fast verification mode, not wide search. It checks whether BDE_opt can stably beat BDE.py under leakage-safe repeated-KFold. Report BDE_opt only if its 25-run mean R2 is higher than BDE.py; otherwise keep BDE.py as final."
)
...
TASK_K_DEFAULTS = {
    ...
    "ALL_TYPES": "800,1200,1600",
    # v10.5: expanded K search around and beyond the v10.3 optimum k=1200.
    "PC_PN": "1200,1400",
}
...
def model_pool(seed: int, task_name: str = "pc_pn") -> Dict[str, object]:
    """v10.5 wider R2-search model pool.

    Purpose:
      - v10.3 used a slim pool (RF_sqrt, ExtraTrees_sqrt, SoftVote) and reached mean R2≈0.772.
      - v10.5 restores a broader search to see whether the dataset can honestly approach 0.90.

    Important:
      - These models are still evaluated only inside the selected split protocol.
      - No test-set calibration, duplicate leakage, or automatic outlier deletion is used.
    """
    n_tree = SLOW_N_TREES if SLOW_TUNING else FAST_N_TREES
    models: Dict[str, object] = {
        "RF_sqrt_local": RandomForestRegressor(
            n_estimators=n_tree, max_features="sqrt", min_samples_leaf=1,
    ...
    if WIDE_MODEL_POOL:
        models.update({
            "RF_040_local": RandomForestRegressor(
                n_estimators=n_tree, max_features=0.40, min_samples_leaf=1,
                random_state=seed, n_jobs=-1
            ),
            "RF_060_local": RandomForestRegressor(
                n_estimators=n_tree, max_features=0.60, min_samples_leaf=1,
                random_state=seed, n_jobs=-1
            ),
            "ExtraTrees_040_local": ExtraTreesRegressor(
                n_estimators=n_tree, max_features=0.40, min_samples_leaf=1,
                random_state=seed, n_jobs=-1
            ),
            "ExtraTrees_060_local": ExtraTreesRegressor(
                n_estimators=n_tree, max_features=0.60, min_samples_leaf=1,
                random_state=seed, n_jobs=-1
            ),
            "GBDT_shallow_local": GradientBoostingRegressor(
                n_estimators=220 if SLOW_TUNING else 130,
                learning_rate=0.035 if SLOW_TUNING else 0.055,
                max_depth=2,
                subsample=0.90,
                random_state=seed,
            ),
            "GBDT_local": GradientBoostingRegressor(
                n_estimators=320 if SLOW_TUNING else 180,
                learning_rate=0.025 if SLOW_TUNING else 0.045,
                max_depth=3,
                subsample=0.85,
                random_state=seed,
            ),
            "HistGB_local": HistGradientBoostingRegressor(
                max_iter=350 if SLOW_TUNING else 180,
                learning_rate=0.025 if SLOW_TUNING else 0.055,
                max_leaf_nodes=15,
    ...
    if USE_XGB and RUN_STANDALONE_XGB:
        models.update({
            "XGB_shallow_local": XGBRegressor(
                n_estimators=420 if SLOW_TUNING else 240,
                learning_rate=0.030 if SLOW_TUNING else 0.045,
                max_depth=2,
                subsample=0.92,
            ...
            "XGB_local": XGBRegressor(
                n_estimators=650 if SLOW_TUNING else 300,
                learning_rate=0.020 if SLOW_TUNING else 0.040,
                max_depth=3,
                subsample=0.90,
    ...
    if RUN_SLOW_KERNEL:
        models.update({
            "SVR_rbf_scaled": Pipeline([
                ...
                ("svr", SVR(C=30.0, epsilon=0.05, gamma="scale")),
            ]),
            "KernelRidge_rbf_scaled": Pipeline([
                ("scaler", StandardScaler(with_mean=False)),
                ("krr", KernelRidge(alpha=1.0, kernel="rbf", gamma=0.001)),
            ]),
        })

    vote_estimators = [
        ("rf", RandomForestRegressor(
            n_estimators=max(90, n_tree // 2), max_features="sqrt", min_samples_leaf=1,
            random_state=seed, n_jobs=-1,
        )),
        ...
        ("gb", GradientBoostingRegressor(
            n_estimators=180 if SLOW_TUNING else 100,
            learning_rate=0.045,
            max_depth=3,
            subsample=0.85,
            random_state=seed,
    ...
    vote_weights = [0.30, 0.40, 0.15]

    if USE_XGB:
        vote_estimators.append(("xgb", XGBRegressor(
            n_estimators=420 if SLOW_TUNING else 220,
            learning_rate=0.032,
            max_depth=3,
            subsample=0.90,
            colsample_bytree=0.78,
        ...
        vote_weights.append(0.15)

    models["SoftVote_local"] = VotingRegressor(vote_estimators, n_jobs=None)
    if USE_WEIGHTED_VOTE:
        models["WeightedVote_local"] = VotingRegressor(vote_estimators, weights=vote_weights, n_jobs=None)

    if USE_STACKING:
        stack_estimators = [
            ("rf", RandomForestRegressor(
                n_estimators=max(80, n_tree // 3), max_features="sqrt", min_samples_leaf=1,
                random_state=seed, n_jobs=-1,
            )),
            ("et", ExtraTreesRegressor(
                n_estimators=max(80, n_tree // 3), max_features="sqrt", min_samples_leaf=1,
        ...
        models["Stacking_Ridge_meta"] = StackingRegressor(
    ...
    if R2_SEARCH_LEVEL in {"slim", "fast"}:
        keep = {"RF_sqrt_local", "ExtraTrees_sqrt_local", "SoftVote_local", "WeightedVote_local"}
        models = {k: v for k, v in models.items() if k in keep and v is not None}
...
def main(argv: Optional[List[str]] = None) -> None:
    ...
    print("=" * 78)
    print("[BDE_opt | fast verification tool, not thesis final model]")
    print("=" * 78)
    ...
    print(f"[INFO] R2_search_level={R2_SEARCH_LEVEL}, wide_model_pool={WIDE_MODEL_POOL}, stacking={USE_STACKING}, weighted_vote={USE_WEIGHTED_VOTE}, standalone_xgb={RUN_STANDALONE_XGB}")
